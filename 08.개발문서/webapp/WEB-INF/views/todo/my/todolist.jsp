            ...
            <table class="listtbl">
                ...
                <c:forEach items="${list}" var="dto">
                  ...
                  <tr>
                    ...
                    <td>${dto.emname }</td>
                   	<td>
                   		<c:if test = "${dto.per==100}">
                   		완료
                   		</c:if>
                   		<c:if test = "${dto.per==0}">
                    	시작전
                    	</c:if>
                    	<c:if test = "${dto.per>0 && dto.per<100}">
                    	진행중
                    	</c:if>
                   	</td>
                    <td>${dto.start}</td>
    ...
    var manageMenu = document.querySelector('.manage');
            ...
            $.ajax({
                ...
                success: (result) => {
                	...
                	if(result.result == 1){
                        ...
                        if(per==100){
                        	$(btn).parent().parent().parent().children().eq(4).text("완료");
                        }
                        else if(per==0) {
                        	$(btn).parent().parent().parent().children().eq(4).text("시작전");
                        }
                        else if(per<100 && per >0) {
                        	$(btn).parent().parent().parent().children().eq(4).text("진행중");
                        }
        ...
        $(document).ready(function(){
        	...
        	$("#pjseq").change(function(){
                    ...
                    success: (result) => {
                    	...
                    	$(result).each((index, item)=>{
                    		...
                    		if (item.per == 100) {
                    			...
                    			temp += '시작전';
                    		} else if (item.per > 0 && item.per<100 ) {
                    			temp += '진행중';
                    		}
                    		...
                    		temp += `</td>
                           		...
                           		<td>
                                    ...
                                    <div style="display: none;">
                                        <input type="button" value="확인" onclick="editok(\${item.seq});" >
        		...
        		$.ajax({
                    type: 'POST',
                    url: '/pms/todo/my/todopagination.do',
                    data: {
                        pjseq: pjseq
                    },
                    dataType:'html',
